...
function renderFirstPublishChecklist(input: {
  ...
  return [
    ...
    "- Generate or attach images from the dashboard with `生成便利贴封面`, or place real photos in an ignored local folder.",
    "- Export the copy package from the dashboard if a human publisher needs a separate Markdown file.",
    "",
    "## Real Account Preflight",
    "",
    "- Click `账号预检` in the dashboard.",
    ...
    "- If selectors are missing, update `config/xhs-selectors.json`, then click `账号预检` again.",
    "- CLI fallback: `npm.cmd run publish:preflight`.",
    "",
    "## Assisted Publish",
    "",
    "- Click `打开小红书发布` in the dashboard.",
    "- CLI fallback if using an image folder: `npm.cmd run publish -- --post latest --images-dir .\\assets\\xhs`.",
