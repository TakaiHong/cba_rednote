...
运营人员在运营台顶部跟随一个主按钮完成三步：生成封面、账号预检、确认并发布。预检窗口在记录页面证据后会自动关闭，不需要按 Enter；通过后，最终发布仍会弹出一次确认框。确认后，后台复用登录态，直达“上传图文”页，上传绑定封面、填写标题正文并点击发布。上面的 CLI 命令仅作为技术兜底。

点击确认后保持运营台打开。系统会轮询本地发布任务；按钮显示“已点击，等待链接”代表小红书发布按钮已经被自动点击，不能再次重复点击。若脚本失败，运营台会显示具体错误。

如需先人工检查小红书编辑页，点击“只填充，不发布”。浏览器会保持打开，人工关闭窗口后脚本自动结束。
...
6. 再次点击“账号预检”，确认通过后点击“确认并发布”。预检报告超过 24 小时会自动失效，需重新预检。
