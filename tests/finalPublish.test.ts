...
describe("final publish guard", () => {
  it("requires both a CLI request and an environment opt-in", () => {
    ...
    assert.equal(shouldAttemptFinalPublish(true, { XHS_ALLOW_FINAL_PUBLISH: "true" }, true), true);
  });

  it("explains why the final click is skipped", () => {
    assert.equal(finalPublishGuardMessage(false), "Final publish click not requested.");
    assert.equal(
      finalPublishGuardMessage(true, { XHS_ALLOW_FINAL_PUBLISH: "false" }),
      "Final publish click requested but blocked. Set XHS_ALLOW_FINAL_PUBLISH=true to enable it."
    );
    assert.equal(
      finalPublishGuardMessage(true, { XHS_ALLOW_FINAL_PUBLISH: "true" }, false),
      "Final publish click requested but blocked. Run npm.cmd run publish:preflight and confirm selector evidence first."
    );
  });
});
